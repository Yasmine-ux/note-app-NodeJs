...
if (process.argv[2].toString()==='noteList'){
    fs.readFile('./noteList.json', 'utf8', function(erreur,data)
    {
        if (erreur)
        throw erreur;
    ...
    fs.readFile('./noteList.json', 'utf8', function (erreur, data){
        ...
        fs.writeFile('./noteList.json', JSON.stringify(monJson), function (err) {
    ...
    fs.readFile('./noteList.json', 'utf8', function (erreur, data){
        ...
        fs.writeFile('./noteList.json', JSON.stringify(newlist), function (err) {
            if (err) return console.log(err);
          });
